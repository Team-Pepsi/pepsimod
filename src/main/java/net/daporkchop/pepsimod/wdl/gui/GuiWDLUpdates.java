...
import net.daporkchop.pepsimod.wdl.update.Release;
...
/**
 * Gui that lists updates fetched via {@link net.daporkchop.pepsimod.wdl.update.GithubInfoGrabber}.
 */
public class GuiWDLUpdates extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!WDLUpdateChecker.hasFinishedUpdateCheck()) {
            drawCenteredString(fontRenderer,
                    I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.pleaseWait"), width / 2,
                    height / 2, 0xFFFFFF);
        } else if (WDLUpdateChecker.hasUpdateCheckFailed()) {
            String reason = WDLUpdateChecker.getUpdateCheckFailReason();

            drawCenteredString(fontRenderer,
                    I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.checkFailed"), width / 2,
                    height / 2 - fontRenderer.FONT_HEIGHT / 2, 0xFF5555);
            drawCenteredString(fontRenderer, I18n.format(reason), width / 2,
                    height / 2 + fontRenderer.FONT_HEIGHT / 2, 0xFF5555);
        }

        drawCenteredString(fontRenderer, I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.title"),
                width / 2, 8, 0xFFFFFF);
    ...
    private String buildVersionInfo(Release release) {
        ...
        return I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.update.version", type, supportedVersions, supportedVersions);
    }
    ...
    private String buildSupportedVersions(String[] versions) {
        ...
        if (versions.length == 1) {
            supportedVersions = I18n.format(
                    "net.daporkchop.pepsimod.wdl.gui.updates.update.version.listSingle",
                    versions[0]);
        } else if (versions.length == 2) {
            supportedVersions = I18n.format(
                    "net.daporkchop.pepsimod.wdl.gui.updates.update.version.listDouble",
            ...
            for (int i = 0; i < versions.length; i++) {
                if (i == 0) {
                    builder.append(I18n.format(
                            "net.daporkchop.pepsimod.wdl.gui.updates.update.version.listStart",
                            versions[i]));
                } else if (i == versions.length - 1) {
                    builder.append(I18n.format(
                            "net.daporkchop.pepsimod.wdl.gui.updates.update.version.listEnd",
                            versions[i]));
                } else {
                    builder.append(I18n.format(
                            "net.daporkchop.pepsimod.wdl.gui.updates.update.version.listMiddle",
                            versions[i]));
                }
            }
    ...
    private String buildReleaseTitle(Release release) {
        ...
        if (release.prerelease) {
            return I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.update.title.prerelease", version, mcVersion);
        } else {
            return I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.update.title.release", version, mcVersion);
        }
    }
    ...
    private class UpdateList extends GuiListExtended {
        ...
        private class VersionEntry extends GuiListEntry {
            ...
            public VersionEntry(Release release) {
                ...
                time = I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.update.releaseDate", release.date);
            }
            ...
            public void drawEntry(int slotIndex, int x, int y, int listWidth,
                ...
                if (isSelected(slotIndex)) {
                    title = I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.currentVersion",
                            this.title);
                } else if (this.release == recomendedRelease) {
                    title = I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.recomendedVersion",
                            this.title);
                } else {
                    title = this.title;
                }
    ...
    private class GuiWDLSingleUpdate extends GuiScreen {
        ...
        public void initGui() {
            this.buttonList.add(new GuiButton(0, this.width / 2 - 155,
                    18, 150, 20, I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.update.viewOnline")));
            if (release.hiddenInfo != null) {
                this.buttonList.add(new GuiButton(1, this.width / 2 + 5,
                        18, 150, 20, I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.update.viewForumPost")));
            }
            this.buttonList.add(new GuiButton(100, this.width / 2 - 100,
                    this.height - 29, I18n.format("gui.done")));
            ...
            list.addLine(I18n.format("net.daporkchop.pepsimod.wdl.gui.updates.update.releaseDate", release.date));
