...
import net.daporkchop.pepsimod.wdl.api.IPlayerInfoEditor;
import net.daporkchop.pepsimod.wdl.api.ISaveListener;
import net.daporkchop.pepsimod.wdl.api.IWorldInfoEditor;
import net.daporkchop.pepsimod.wdl.api.WDLApi;
import net.daporkchop.pepsimod.wdl.api.WDLApi.ModInfo;
import net.daporkchop.pepsimod.wdl.gui.GuiWDLMultiworld;
import net.daporkchop.pepsimod.wdl.gui.GuiWDLMultiworldSelect;
import net.daporkchop.pepsimod.wdl.gui.GuiWDLOverwriteChanges;
import net.daporkchop.pepsimod.wdl.gui.GuiWDLSaveProgress;
import net.daporkchop.pepsimod.wdl.update.GithubInfoGrabber;
...
public class WDL {
    ...
    public static void startDownload() {
        ...
        if (isMultiworld && worldName.isEmpty()) {
            ...
            minecraft.displayGuiScreen(new GuiWDLMultiworldSelect(I18n
                    .format("net.daporkchop.pepsimod.wdl.gui.multiworldSelect.title.startDownload"),
        ...
        WDLMessages.chatMessageTranslated(WDLMessageTypes.INFO,
                "net.daporkchop.pepsimod.wdl.messages.generalInfo.downloadStarted");
    }
    ...
    public static void stopDownload() {
        if (downloading) {
            ...
            startOnChange = false;
            WDLMessages.chatMessageTranslated(WDLMessageTypes.INFO,
                    "net.daporkchop.pepsimod.wdl.messages.generalInfo.downloadStopped");
            startSaveThread();
        }
    }
    ...
    public static void cancelDownload() {
        ...
        if (wasDownloading) {
            ...
            WDLMessages.chatMessageTranslated(WDLMessageTypes.INFO,
                    "net.daporkchop.pepsimod.wdl.messages.generalInfo.downloadCanceled");
        }
    }
    ...
    static void startSaveThread() {
        ...
        WDLMessages.chatMessageTranslated(WDLMessageTypes.INFO,
                "net.daporkchop.pepsimod.wdl.messages.generalInfo.saveStarted");
        WDL.saving = true;
    ...
    public static boolean loadWorld() {
        ...
        if (networkManager != newNM) {
            ...
            WDLMessages.chatMessageTranslated(WDLMessageTypes.ON_WORLD_LOAD,
                    "net.daporkchop.pepsimod.wdl.messages.onWorldLoad.differentServer");
            ...
            if (isSpigot()) {
                WDLMessages.chatMessageTranslated(
                        ...
                        "net.daporkchop.pepsimod.wdl.messages.onWorldLoad.spigot",
                        thePlayer.getServerBrand());
            } else {
                WDLMessages.chatMessageTranslated(
                        WDLMessageTypes.ON_WORLD_LOAD,
                        "net.daporkchop.pepsimod.wdl.messages.onWorldLoad.vanilla",
                        thePlayer.getServerBrand());
            }
            ...
            WDLMessages.chatMessageTranslated(WDLMessageTypes.ON_WORLD_LOAD,
                    "net.daporkchop.pepsimod.wdl.messages.onWorldLoad.sameServer");
    ...
    public static void onSaveComplete() {
        ...
        if (downloading) {
            WDLMessages.chatMessageTranslated(WDLMessageTypes.INFO,
                    "net.daporkchop.pepsimod.wdl.messages.generalInfo.saveComplete.startingAgain");
            WDL.loadWorld();
            return;
        }

        WDLMessages.chatMessageTranslated(WDLMessageTypes.INFO,
                "net.daporkchop.pepsimod.wdl.messages.generalInfo.saveComplete.done");
    }
    ...
    public static void saveEverything() throws Exception {
        if (!WDLPluginChannels.canDownloadAtAll()) {
            WDLMessages.chatMessageTranslated(WDLMessageTypes.ERROR,
                    "net.daporkchop.pepsimod.wdl.messages.generalError.forbidden");
            return;
        }
        ...
        final GuiWDLSaveProgress progressScreen = new GuiWDLSaveProgress(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.title"),
        ...
        for (ModInfo<ISaveListener> info : WDLApi
            ...
            progressScreen.startMajorTask(
                    I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.extension.title",
        ...
        try {
            WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                    "net.daporkchop.pepsimod.wdl.messages.saving.flushingIO");

            progressScreen.startMajorTask(
                    I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.flushingIO.title"), 1);
            progressScreen.setMinorTaskProgress(() -> {
                return I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.flushingIO.subtitle", chunkLoader.getNumPendingChunks());
            }, 1);
        ...
        if (backupType != WorldBackupType.NONE) {
            WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                    "net.daporkchop.pepsimod.wdl.messages.saving.backingUp");
            progressScreen.startMajorTask(
                    backupType.getTitle(), 1);
            progressScreen.setMinorTaskProgress(
                    I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.backingUp.preparing"), 1);
            ...
            class BackupState implements WorldBackup.IBackupProgressMonitor {
                ...
                public void onNextFile(String name) {
                    progressScreen.setMinorTaskProgress(
                            I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.backingUp.file", name), curFile++);
                }
            }
            ...
            try {
                ...
                WDLMessages.chatMessageTranslated(WDLMessageTypes.ERROR,
                        "net.daporkchop.pepsimod.wdl.messages.generalError.failedToBackUp");
            }
        }
    ...
    public static NBTTagCompound savePlayer(GuiWDLSaveProgress progressScreen) {
        ...
        progressScreen.startMajorTask(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.playerData.title"),
                3 + WDLApi.getImplementingExtensions(IPlayerInfoEditor.class).size());
        WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                "net.daporkchop.pepsimod.wdl.messages.saving.savingPlayer");

        progressScreen.setMinorTaskProgress(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.playerData.creatingNBT"), 1);

        NBTTagCompound playerNBT = new NBTTagCompound();
        thePlayer.writeToNBT(playerNBT);

        progressScreen.setMinorTaskProgress(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.playerData.editingNBT"), 2);
        ...
        for (ModInfo<IPlayerInfoEditor> info : WDLApi
            ...
            progressScreen.setMinorTaskProgress(
                    I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.playerData.extension",
        ...
        progressScreen.setMinorTaskProgress(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.playerData.writingNBT"), taskNum);
        ...
        WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                "net.daporkchop.pepsimod.wdl.messages.saving.playerSaved");
    ...
    public static void saveWorldInfo(GuiWDLSaveProgress progressScreen,
        ...
        progressScreen.startMajorTask(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.worldMetadata.title"),
                3 + WDLApi.getImplementingExtensions(IWorldInfoEditor.class).size());
        WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                "net.daporkchop.pepsimod.wdl.messages.saving.savingWorld");

        progressScreen.setMinorTaskProgress(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.worldMetadata.creatingNBT"), 1);
        ...
        progressScreen.setMinorTaskProgress(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.worldMetadata.editingNBT"), 2);
        ...
        for (ModInfo<IWorldInfoEditor> info : WDLApi
            ...
            progressScreen.setMinorTaskProgress(
                    I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.worldMetadata.extension",
                            info.getDisplayName()), taskNum);
        ...
        progressScreen.setMinorTaskProgress(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.worldMetadata.writingNBT"), taskNum);
        ...
        WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                "net.daporkchop.pepsimod.wdl.messages.saving.worldSaved");
    }
    ...
    public static void saveChunks(GuiWDLSaveProgress progressScreen)
        ...
        WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                "net.daporkchop.pepsimod.wdl.messages.saving.savingChunks");
        ...
        progressScreen.startMajorTask(I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.chunk.title"),
                chunks.size());
        ...
        for (int currentChunk = 0; currentChunk < chunks.size(); currentChunk++) {
            ...
            if (c != null) {
                ...
                progressScreen.setMinorTaskProgress(I18n.format(
                        "net.daporkchop.pepsimod.wdl.saveProgress.chunk.saving", c.x,
                        c.z), currentChunk);
        ...
        WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                "net.daporkchop.pepsimod.wdl.messages.saving.chunksSaved");
    }
    ...
    public static void saveChunk(Chunk c) {
        ...
        try {
            ...
            WDLMessages.chatMessageTranslated(WDLMessageTypes.ERROR,
                    "net.daporkchop.pepsimod.wdl.messages.generalError.failedToSaveChunk",
                    c.x, c.z, e);
        }
    }
    ...
    public static void saveProps(String theWorldName, Properties theWorldProps) {
        ...
        if (theWorldName.length() > 0) {
            ...
            try (FileWriter writer = new FileWriter(worldPropsFile)) {
                theWorldProps.store(writer, I18n.format("net.daporkchop.pepsimod.wdl.props.world.title"));
            } catch (Exception e) {
        ...
        try (FileWriter writer = new FileWriter(basePropsFile)) {
            baseProps.store(writer, I18n.format("net.daporkchop.pepsimod.wdl.props.base.title"));
        } catch (Exception e) {
    ...
    public static void saveGlobalProps() {
        ...
        try (FileWriter writer = new FileWriter(globalPropsFile)) {
            globalProps.store(writer, I18n.format("net.daporkchop.pepsimod.wdl.props.global.title"));
        } catch (Exception e) {
    ...
    public static void saveMapData(GuiWDLSaveProgress progressScreen) {
        ...
        progressScreen.startMajorTask(
                I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.map.title"), newMapDatas.size());

        WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                "net.daporkchop.pepsimod.wdl.messages.saving.savingMapItemData");
        ...
        for (Map.Entry<Integer, MapData> e : newMapDatas.entrySet()) {
            ...
            progressScreen.setMinorTaskProgress(
                    I18n.format("net.daporkchop.pepsimod.wdl.saveProgress.map.saving", e.getKey()),
                    count);
        ...
        WDLMessages.chatMessageTranslated(WDLMessageTypes.SAVING,
                "net.daporkchop.pepsimod.wdl.messages.saving.mapItemDataSaved");
    }
     ...
     *
     * @param container           The container to save from, usually {@link #windowContainer} .
     * @param tileEntity          The TileEntity to save to.
     * @param containerStartIndex The index in the container to start copying items from.
     */
     ...
     * @param te  The tile entity to add
     */
    ...
    private static void runSanityCheck() {
        ...
        if (!failures.isEmpty()) {
            WDLMessages.chatMessageTranslated(WDLMessageTypes.ERROR, "net.daporkchop.pepsimod.wdl.sanity.failed");
